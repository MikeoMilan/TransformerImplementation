...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None): # q -> batch * length(token+padding) * d_model(vocab_size)
        d_k, d_v, n_head = self.d_k, self.d_v, self.n_head
        sz_b, len_q, len_k, len_v = q.size(0), q.size(1), k.size(1), v.size(1)

        residual = q

        # batch * length * vocab_size ----project---->  batch * length * (n_head*d_k) ----view----> batch * length * n_head * d_k
